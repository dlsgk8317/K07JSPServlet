...
<!-- 
	...
	만약 파일을 첨부하지 않은 상태로 수정을 하게되면, 기존 파일명을
	그대로 유지해야 하므로 hidden 입력상자를 통해 기존 파일명을
	전송한다.
 -->
<form name="writeFrm" method="post" enctype="multipart/form-data"
	action="../mvcboard/edit.do" onsubmit="return formValidate(this)">
<input type="hidden" name="idx" value="${dto.idx }" /><!-- 일련번호 -->
<input type="hidden" name="prevOfile" value="${dto.ofile }" /> <!-- 원본파일명 -->
<input type="hidden" name="prevSfile" value="${dto.sfile }" /> <!-- 저장된 파일명 -->
